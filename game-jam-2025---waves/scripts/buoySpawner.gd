...
@export var buoy_spacing: float = 10.0
...
func create_wall_side_mesh(perpendicular_offset: float, wall_name: String):
	...
	for i in range(num_points):
		var offset = (i / float(num_points - 1)) * curve_length
		...
		if i < num_points - 2: # - 2 avoids loop issue where it blocks off route, fix later
			var next_offset = ((i + 1) / float(num_points - 1)) * curve_length
